...
class OllamaClient:
    """Client for interacting with Ollama local model for structured product data."""
    ...
    def _build_prompt(
        ...
        return (
            "Analyze the following food ingredient/supplement raw material and return ONLY a valid JSON object with exactly these fields.\n"
            "Product Information:\n"
            f"- Original Name: {original_product_name}\n"
            f"- Cleaned Name: {cleaned_product_name}\n"
            f"- Supplier: {supplier_name}\n"
            f"- Additional Data: {supplier_data_text}\n\n"
            "REQUIRED JSON FIELDS (return ONLY JSON, no other text):\n"
            "- cleaned_canonical_name: string\n"
            f"- general_class: one of [{classes_str}]\n"
            "- ingredient_type: string\n"
            "- functional_role: string\n"
            "- physical_form: string\n"
            "- application_domain: string\n"
            "- synonyms: array of strings\n"
            "- short_embedding_text: string (max 100 chars)\n"
